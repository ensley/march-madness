...
from march_madness import transform, tournament
from march_madness.stan import Season, Bracket
...
@click.option("--outdir", type=click.Path(dir_okay=True, exists=False, path_type=pathlib.Path), default=pathlib.Path("."))
def fit_season(games_file: pathlib.Path, year: int, outdir: pathlib.Path):
    outdir: pathlib.Path = outdir / f"march-madness-season-{datetime.now().strftime('%Y%m%d%H%M%S')}"

    df, teams, data = transform.prep_season_data(games_file, year, games_through="ctourn")
    season = Season.sample(data=data)

    outdir.mkdir(parents=True, exist_ok=True)
    season.save_csvfiles(dir=outdir)
...
def simulate_tournament(model_dir: pathlib.Path, tourney_file: pathlib.Path):
    season = cmdstanpy.from_csv(os.fspath(model_dir) + "/season-*.csv")
    ...
    outcomes = (
        pd.DataFrame(bracket.bracket.mean(axis=0).T, index=tourney)
